...
def main():
    schema_path = os.path.join(os.path.dirname(__file__), 'entity-schema.json')
    ...
    json_pattern = os.path.join(base_dir, 'data', 'entities', '*.json')
    json_files = sorted(glob.glob(json_pattern))

    if not json_files:
        print("Warning: No entity JSON files found under 'data/entities/'.")
        sys.exit(0)

    success = True
    print(f"Found {len(json_files)} entity file(s) to validate.\n")
    ...
    for file_path in json_files:
        ...
        if errors:
            ...
            print(f"  [OK] {relative_path} is valid ({len(data)} records).")
